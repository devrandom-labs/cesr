...
mod tests {
    ...
    fn prefixer(raw: [u8; 32]) -> Prefixer<'static> {
        MatterBuilder::new()
    ...
    fn saider(raw: [u8; 32]) -> Saider<'static> {
        MatterBuilder::new()
    ...
    type IdSpec = (bool, [u8; 32]);
    /// (variant selector, raw a, raw b, sn) -> Seal
    type SealSpec = (u8, [u8; 32], [u8; 32], u128);
    /// (simple?, simple value, weighted clauses) -> Tholder
    type TholderSpec = (bool, u64, Vec<Vec<(u64, u64)>>);
    type IcpSpec = (
        IdSpec,
        u128,
        [u8; 32],
    ...
    type IxnSpec = (IdSpec, u128, [u8; 32], [u8; 32], Vec<SealSpec>);

    fn build_identifier((basic, raw): IdSpec) -> Identifier<'static> {
        if basic {
    ...
    fn build_seal((variant, a, b, sn): SealSpec) -> Seal {
        match variant {
    ...
    fn build_tholder((simple, value, clauses): TholderSpec) -> Tholder {
        if simple {
    ...
    fn build_config(picks: &[bool]) -> Vec<ConfigTrait> {
        picks
            .iter()
            .map(|p| {
                if *p {
    ...
    fn build_icp(spec: IcpSpec) -> InceptionEvent {
    ...
    fn build_rot(spec: RotSpec) -> RotationEvent {
    ...
    fn build_ixn(spec: IxnSpec) -> InteractionEvent {
    ...
    fn sn_strategy() -> impl Strategy<Value = u128> {
        prop_oneof![Just(0_u128), Just(1_u128), Just(u128::MAX), any::<u128>()]
    }

    fn bt_strategy() -> impl Strategy<Value = u32> {
        prop_oneof![Just(0_u32), Just(1_u32), Just(u32::MAX), any::<u32>()]
    }

    fn tholder_strategy() -> impl Strategy<Value = TholderSpec> {
        (
            any::<bool>(),
    ...
    fn seal_strategy() -> impl Strategy<Value = SealSpec> {
        (0_u8..5, any::<[u8; 32]>(), any::<[u8; 32]>(), sn_strategy())
    }

    fn icp_strategy() -> impl Strategy<Value = IcpSpec> {
        (
            any::<IdSpec>(),
            sn_strategy(),
            any::<[u8; 32]>(),
    ...
    fn rot_strategy() -> impl Strategy<Value = RotSpec> {
        (
            any::<IdSpec>(),
            sn_strategy(),
            any::<[u8; 32]>(),
    ...
    fn ixn_strategy() -> impl Strategy<Value = IxnSpec> {
        (
            any::<IdSpec>(),
            sn_strategy(),
            any::<[u8; 32]>(),
