...
- *(codec)* TEL codec — public `TelEvent` deserialization, framed `TelMessage`
  ...
  `RevokeBuilder`, `BackedIssueBuilder`, `BackedRevokeBuilder`) mirroring
  keripy's `vdr/eventing.py` factories, all over the generic SAD
  saidify/verify spine from #292. ACDC credential bodies ride the same
  generic path — no TEL-specific digest logic.
- *(codec)* keripy TEL differential corpus (`tests/corpus/tel/`, generated by
  `scripts/keripy_tel_gen.py`): 7 happy-path events, 7 issuer-signed
  messages, 10 parse-hardening rejects — hand-authored shape reproductions
  of the pinned keripy (partial; the pin requires Python >= 3.14), replayed
  byte-identically through the public API with verified indexed signatures
  by `tests/keripy_tel.rs`.
- *(parity)* lifted the `vcp`/`vrt`/`iss`/`rev`/`bis`/`brv` divergence
  markers in `tests/corpus/keripy/parity/codex.jsonl` — the ilk rows now
  assert in the codex sweep (`exn` stays marked until the IPEX lane).
