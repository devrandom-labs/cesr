...
pub(crate) trait RenderBody {
    ...
    /// (`KERI10JSON000000_`) and a placeholder in every SAID slot; the
    /// orchestration in [`EventRef::serialize`] backpatches the measured size,
...
impl EventRef<'_> {
    /// Serialize this event through the single canonical writer: render once
    /// with a placeholder SAID and zero-size version string, backpatch the
    /// measured size in place, compute the SAID over the size-corrected bytes,
    /// and splice it into the reported slot(s).
    ///
    ...
    pub(crate) fn serialize(self) -> Result<SerializedEvent, CodecError> {
        ...
        // first would corrupt `i`'s digest. The render IS the dummied state:
        // every SAID slot carries its code's placeholder, so the shared SAID
        // core digests it directly. Digests land in stack locals — no
        // intermediate tree or render between buffer and splice (allocation
        // shape pinned by the serder_allocation test).
        if layout.prefix.is_some() != prefix_code.is_some() {
...
mod tests {
    ...
    mod icp {
        ...
        fn serialize_icp_basic_prefix_verbatim_single_said() {
            ...
            let computed = Saider::digest(DigestCode::Blake3_256, reser.as_bytes()).unwrap();
            assert_eq!(
                d,
                computed.to_qb64(),
    ...
    mod rot {
        ...
        fn serialize_rot_said_is_valid() {
            ...
            crate::said::verify_said_raw(result.as_bytes()).expect("SAID verification should pass");
        }
