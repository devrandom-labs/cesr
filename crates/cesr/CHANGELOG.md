...
- *(matter)* `MatterBuilder::from_qualified_base64` decode seam does less redundant work per call. It resolved the `code -> Sizage` descriptor by re-running the ~110-variant `get_sizage()` match six times (once per field access, including inside `frame_size_of`), and allocated a fresh `PAD.repeat(xs)` `String` on every decode just to compare the xtra bytes. The descriptor is now resolved once and reused; the xtra check compares bytes directly (`xtra.iter().all(|&b| b == b'_')`, `PAD == "_"`); and `frame_size_of` is inlined so the soft tail is UTF-8-validated and `decode_int`-parsed exactly once (fixed codes still validate UTF-8, matching prior behavior). Behavior and error variants are byte-identical for every input. Measured (`matter_decode` bench, `cesr-stream`): ~7% faster on the variable-size decode path (`strb64_l0_variable` 71.8 → 66.7 ns, where the `PAD.repeat` allocation is dropped); the fixed-size path (`blake3_256_fixed`) is within noise (~1%).
- *(b64)* `encode_int` now performs a single heap allocation instead of two. It previously filled a `vec![b'A'; len]` least-significant-digit-first and then re-encoded it through `.into_iter().map(char::from).collect()` — a second allocation, because `char` widens each ASCII byte to 4 bytes before `String` re-narrows it. Digits are now written most-significant-first straight into one `String::with_capacity(len)` (exact capacity, never reallocates), and the per-digit `x /= 64` divide becomes a `val >> (6 * pos)` shift. Measured on the new `b64_int` bench: the 2-char count-code case goes 34.98 ns → 15.17 ns (2.31× faster); `decode_int` is untouched and unchanged at ~1.42 ns. No API, signature, or output change — `encode_int` remains infallible and byte-identical for every input; the `u64` roundtrip proptest was widened from `0..68_719_476_736` to the full `0..=u64::MAX` domain and boundary vectors up to `u64::MAX` were added.
- *(indexer)* [**breaking**] `IndexerParseError` gains a `SizeOverflow` variant. `IndexerBuilder::from_qb64`/`from_qb2` previously computed the variable-code full size with bare `index * 4 + cs`; they now route through a checked `compute_full_size` and return `SizeOverflow` on overflow, matching the Matter side's hardening. Downstream matches on `IndexerParseError` become non-exhaustive.
- *(counter)* [**breaking**] `CounterCodeError` gains `NotACounter` and `StreamTooShort { need }` variants, so `CounterCodeV1`/`CounterCodeV2::from_base64_stream` now distinguish "truncated — buffer more bytes" (`StreamTooShort`) from "not a counter head" (`NotACounter`) from "unknown code" (`UnknownCode`), instead of collapsing all three into `UnknownCode`. `stream_hard_size` returns to a `const fn`. Downstream matches on `CounterCodeError` become non-exhaustive and must handle the new variants; the `cesr-stream` framer now maps these variants directly rather than re-deriving the counter hard-size grammar.
- *(keri)* [**breaking**] workspace split phase 3 (#192, final) — the `keri` module is extracted into a new `keri-events` crate. `cesr::keri::X` is now `keri_events::X`; the `keri` and `internals` features are removed (`internals` moves to `keri-events`). `cesr::prelude` no longer re-exports `ConfigTrait` / `Identifier` / `KeriEvent`, and the crate-root re-exports of the keri vocabulary are gone. `cesr` gains a dev-dependency on `keri-events` for one `crypto` threshold test (a Cargo dev-cycle, permitted). No promotions were needed — `keri` reached only public `core` items. `cesr` is now the CESR substrate alone: `b64` + `core` + `crypto`. This completes the workspace split into `cesr` / `cesr-stream` / `keri-events` / `keri-codec`. No wire behavior changed; keripy differential and spine byte-identity suites pass unchanged.
- *(stream)* [**breaking**] workspace split phase 2 (#192) — the `stream` module is extracted into a new `cesr-stream` crate. `cesr::stream::X` is now `cesr_stream::X`; the `stream` and `async` features are removed (`async` moves to `cesr-stream`). `cesr::prelude` no longer re-exports `CesrEncode` / `CesrGroup` / `CesrMessage`. Two boundary-forced changes to items `stream` reached: `b64::alphabet::{B64_ALPHABET, b64_byte_to_index}` are promoted `pub(crate)` → `pub`, and the `CounterCodeV1` / `CounterCodeV2` count-code tables (in `core::counter`) lose their stray `#[cfg(feature = "stream")]` gate — they are now unconditional `core` primitives, consistent with their ungated siblings `MatterCode` / `IndexerCode`. No wire behavior changed; keripy differential and spine byte-identity suites pass unchanged. Mechanical carve — API redesign deferred to #193.
- *(serder)* [**breaking**] workspace split phase 1 (#192) — the `serder` module is extracted into a new `keri-codec` crate. `cesr::serder::X` is now `keri_codec::X`; the `serder` feature is removed. `cesr::prelude` no longer re-exports `KeriDeserialize` / `KeriSerialize` (they move to `keri_codec::prelude`). Any `cesr::serder` path named in the free-function-collapse entry below is now a `keri_codec` path. Three `cesr` items the codec reached across the new crate boundary are promoted from `pub(crate)` to `pub`: `core::version::VERSION_SIZE_MAX`, `keri::seal::scan_object`, and `stream::cold::ColdCode::detect` — the split surfaced pre-existing coupling rather than creating it. No wire behavior changed; the keripy differential and spine byte-identity suites pass unchanged. This is a mechanical carve — API redesign is deferred to #193.
